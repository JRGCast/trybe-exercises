...
# atribuição (não é necessário a utilização de let , var ou const nas atribuições): 
...
# incrementação 
counter = 0
# Maneiras de incrementar ATENÇÃO => Não existe ++ em python!
...
a=10
b=5
print(f"Soma: {a+b}; Subtração: {a-b}; Multiplicação: {a*b}; Divisão: {a/b}; Divisão Inteira: {a//b}; Potenciação: {a**b}; Módulo: {a%b}")
...
minutes = hours * 60
seconds = minutes * 60
print(f"Minutos em 6 horas: {minutes}; Segundos em 6 horas: {seconds} ")

# Exercício 3: Teste e verifique o que acontece se você colocar um ponto e vírgula no final de uma instrução em Python.
print('Assim como em JavaScript, Python permite que você coloque um ponto e vírgula no final de uma instrução.')
